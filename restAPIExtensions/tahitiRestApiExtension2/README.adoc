...
= tahitiRestApiExtension
REST API extension that provide API to list vacation request submitted by employee managed by provided manager id

The project **tahitiRestApiExtension2** is a Bonita REST API extension for **Bonita 7.12.1** version  **community** written in `groovy`
...
[source,bash]
----
git clone https://your.github.com/tahitiRestApiExtension2.git
cd tahitiRestApiExtension2/
mvn package
----

The build should produce a zip archive under the `target/` folder named `tahitiRestApiExtension2-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `tahitiRestApiExtension2-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see https://documentation.bonitasoft.com/bonita/${shortBonitaVersion}/rest-api-extensions#toc7)
